...
// инициализация темы
onMounted(() => {
  window.addEventListener('click', toggleThemeMenu);
  window.addEventListener('keydown', closeThemeMenu);
  window.addEventListener('storage', changeLocalStorage);

  if (JSON.parse(localStorage.getItem(themeKey))?.mode === 'system') {
    systemTheme();
  } else if (JSON.parse(localStorage.getItem(themeKey))?.mode === 'manual') {
    updateState(JSON.parse(localStorage.getItem(themeKey))?.currentIcon, JSON.parse(localStorage.getItem(themeKey))?.currentTheme, JSON.parse(localStorage.getItem(themeKey))?.mode);
    useHead({ htmlAttrs: { 'data-theme': state.currentTheme, 'data-theme-mode': state.mode } });
  } else {
    systemTheme();
  }
});
...
onUnmounted(() => {
  window.removeEventListener('click', toggleThemeMenu);
...
const toggleThemeMenu = (e) => {
  if (e.target.closest('.theme-switcher__toggle') ) { // клик по кнопке 'тема'
    isOpen.value = !isOpen.value;
  } else if (!e.target.classList.contains('theme-switcher__menu')) { // клик по меню
    isOpen.value = false;
  }
};
...
// слежение за LS (обновляем переключение темы во всех открытых вкладках)
const changeLocalStorage = () => {
  updateState(JSON.parse(localStorage.getItem(themeKey))?.currentIcon, JSON.parse(localStorage.getItem(themeKey))?.currentTheme, JSON.parse(localStorage.getItem(themeKey))?.mode);
  useHead({ htmlAttrs: { 'data-theme': state.currentTheme, 'data-theme-mode': state.mode } });
};

// тема, которая синхронизирована с OS
const systemTheme = () => {
  isDarkTheme.onchange = systemTheme;
  updateState(themeMenu[0].icon, isDarkTheme.matches ? 'dark' : 'light', 'system');
  useHead({ htmlAttrs: { 'data-theme': state.currentTheme, 'data-theme-mode': state.mode } });
  localStorage.setItem(themeKey, JSON.stringify(state));
};

// тема, которую установил пользователь
const manualTheme = (e) => {
  ...
  if (e.currentTarget.getAttribute('data-theme-value') === 'default') {
    ...
    updateState(themeMenu[1].icon, 'light', 'manual');
  } else if (e.currentTarget.getAttribute('data-theme-value') === 'dark') {
    updateState(themeMenu[2].icon, 'dark', 'manual');
  } else if (e.currentTarget.getAttribute('data-theme-value') === 'manual') {
    updateState(themeMenu[3].icon, '--dev', 'manual');
  }
...
// обновление state
const updateState = (currentIcon, currentTheme, mode) => {
  state.currentIcon = currentIcon;
...
<template>
  <div class="theme-switcher">
    ...
    <!-- кнопка -->
    <button :class="['theme-switcher__toggle', { 'theme-switcher__toggle--active': isOpen }]" aria-label="Переключить тему">
      <span class="theme-switcher__toggle-description">Тема:</span>
      <span class="theme-switcher__toggle-icon" v-html="state.currentIcon"></span>
    </button>

    <!-- меню -->
    <Transition name="theme-menu">
      <div class="theme-switcher__menu" aria-label="Меню выбора темы" v-if="isOpen">
        <button class="theme-switcher__menu-btn" v-for="btn in themeMenu" :key="btn.id" :data-theme-value="btn.attr" @click="manualTheme">
          <span class="theme-switcher__menu-description">{{ btn.description }}</span>
          <span class="theme-switcher__menu-icon" v-html="btn.icon"></span>
        </button>
      </div>
  </Transition>
